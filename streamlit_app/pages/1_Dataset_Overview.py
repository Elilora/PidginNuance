...
with col1:
    ...
    df = pd.DataFrame(
        list(data["speaker_gender"].items()), columns=["Gender", "Count"]
    )
...
with col2:
    ...
    df = pd.DataFrame(
        list(data["register"].items()), columns=["Register", "Count"]
    )
...
with col3:
    ...
    df = pd.DataFrame(
        list(data["source_type"].items()), columns=["Source", "Count"]
    )
...
with col1:
    ...
    df = pd.DataFrame(
        list(data["topic_domain"].items()), columns=["Topic", "Count"]
...
with col2:
    ...
    df = pd.DataFrame(
        list(data["intensity_distribution"].items()), columns=["Intensity", "Count"]
    )
...
if "date_trend" in data and data["date_trend"]:
    ...
    df = pd.DataFrame(
        list(data["date_trend"].items()), columns=["Month", "Count"]
    ).sort_values("Month")
